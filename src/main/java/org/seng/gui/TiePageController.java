...
public class TiePageController {
    ...
    public void initialize() {
        tieLabel.setTranslateY(-200); // Start above screen

        // Drop animation
        TranslateTransition dropAnimation = new TranslateTransition(Duration.seconds(2.0), tieLabel);
        dropAnimation.setFromY(-200);
        dropAnimation.setToY(0);
        dropAnimation.setCycleCount(1);

        // Pulse animation
        ScaleTransition pulseAnimation = new ScaleTransition(Duration.seconds(0.8), tieLabel);
    ...
    private void addControllerIcons() {
        ...
        Image controllerImage = new Image(getClass().getResourceAsStream("/org/seng/gui/images/tiePageIcon.png"));

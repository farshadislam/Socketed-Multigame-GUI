...
public class LosingPageController {
    ...
    public void initialize() {
        loseLabel.setTranslateY(-200); // Start above screen

        // Drop animation
        TranslateTransition dropAnimation = new TranslateTransition(Duration.seconds(2.0), loseLabel);
        dropAnimation.setFromY(-200);
        dropAnimation.setToY(0);
        dropAnimation.setCycleCount(1);

        // Pulse animation
        ScaleTransition pulseAnimation = new ScaleTransition(Duration.seconds(0.8), loseLabel);
    ...
    private void addControllerIcons() {
        ...
        Image controllerImage = new Image(getClass().getResourceAsStream("/org/seng/gui/images/wompwomp.png"));
